...
func include(str string, v interface{})[]byte {
	if strings.Contains(str,"{%include "){
		s := "{%include .*%}"
		reg := regexp.MustCompile(s)
		data := reg.FindAll([]byte(str), -1)
		for _,v := range data{

			reg1 := regexp.MustCompile(string(v))
			sdata := string(reg.Find([]byte(str)))
			sdatas := strings.Split(sdata, " ")
			if len(sdatas) < 2 {
				continue
			}
			file := strings.Replace(sdatas[1],"%}", "", -1)
			pwd,_ := os.Getwd()
			tempfile := filepath.Join(pwd, AppConfig.String("viewspath"), file)
			d,err := ioutil.ReadFile(tempfile)
			if err == nil{
				str =	reg1.ReplaceAllString(str, string(d))
			}else{
				logs.Error("模板文件替换失败", err)
			}
		}
	}
	if strings.Contains(str, "{{.username}}"){
		if v != nil {
			str = strings.Replace(str,"{{.username}}", v.(string), -1)
		}
	}
	return []byte(str)
}
...
func (c *Controller) Render() error {
	...
	if c.CruSession != nil &&  c.GetSession("username") != nil {
		str := string(rb)
		rb = include(str, c.GetSession("username"))
	}
