	...
	applicationNs :=
		beego.NewNamespace("/application",
			...
			beego.NSNamespace("/configure",
				...
				// 添加配置页面
				beego.NSRouter("/add", &app.ConfigureController{}, "get:ConfigureAdd","创建配置", "配置管理列表", "配置管理"),
				// 应用详情页面,
				beego.NSRouter("/detail/:hi:*", &app.ConfigureController{}, "get:DetailPage", "配置管理详情", "配置管理列表", "配置管理"),
				// 应用配置添加页面
				beego.NSRouter("/data/add", &app.DataController{}, "post:ConfigDataAdd", "配置数据添加", "配置管理列表", "配置管理"),
			),
			// 容器管理,
			// 容器入口,
