...
func init() {
		...
		beego.NewNamespace("/image",
			...
			beego.NSNamespace("/registry",
				...
				beego.NSNamespace("/perm",
					beego.NSRouter("/list", &registry.RegistryPermController{}, "get:RegistryPermList", "权限管理", "镜像中心", "应用交付中心"),
					// 仓库权限配置页面,
					beego.NSRouter("/add", &registry.RegistryPermController{}, "get:RegistryPermAdd", "添加权限", "权限管理", "权限管理"),
				),
				// 基础镜像
		...
		beego.NewNamespace("/base",
			...
			beego.NSNamespace("/network",
				...
				beego.NSNamespace("/lb",
					beego.NSRouter("/list", &lb.LbController{}, "get:LbList", "负载均衡", "网络管理", "基础设施"),
					// 资源负载均衡器添加页面,
					beego.NSRouter("/add", &lb.LbController{}, "get:LbAdd", "创建负载均衡", "负载均衡", "网络管理"),
					// 集群负载均衡器详情页面,
					beego.NSRouter("/detail/:id:int", &lb.LbController{}, "get:LbDetailPage", "负载均衡详情", "负载均衡", "网络管理"),
					// 负载均衡服务添加页面,
					beego.NSRouter("/service/add", &lb.ServiceController{}, "get:ServiceAdd", "添加负载均衡服务", "负载均衡", "网络管理"),
				),
				// 证书管理
				beego.NSNamespace("/cert",
					// 证书入口
					beego.NSRouter("/list", &lb.CertController{}, "get:CertList", "证书管理", "网络管理", "基础设施"),
					// 证书添加页面,
					beego.NSRouter("/add", &lb.CertController{}, "get:CertAdd", "创建证书", "网络管理", "基础设施"),
				),
			),
			// 服务使用量,
			...
			beego.NSNamespace("/cluster",
				...
				// 集群详情数据页面,
				beego.NSRouter("/detail/:hi(.*)", &cluster.ClusterController{}, "get:DetailPage", "集群详情", "集群列表", "集群管理"),
				// 基础设施管理,
				// 集群主机管理,
				beego.NSRouter("/hosts/list", &hosts.HostsController{}, "get:List", "主机列表", "集群详情", "集群列表"),
				beego.NSRouter("/hosts/add", &hosts.HostsController{}, "get:Add", "添加主机", "集群详情", "集群列表"),
				// 添加主机标签,
				beego.NSRouter("/label/add", &hosts.HostsController{}, "get:LabelAdd", "主机标签添加", "集群详情", "集群列表"),
			),
		)
	...
	baseApi :=
		beego.NewNamespace("/api",
			...
			beego.NSNamespace("/lb",
				// 资源负载均衡器保存,
				beego.NSRouter("", &lb.LbController{}, "post:LbSave", "保存负载均衡", "负载均衡", "基础设施"),
				// 获取资源负载均衡器数据所有数据,
				beego.NSRouter("", &lb.LbController{}, "get:LbData", "获取负载均衡数据", "负载均衡", "基础设施"),
				// 删除负载均衡器,
				beego.NSRouter("/:id:int", &lb.LbController{}, "delete:LbDelete", "删除负载均衡", "负载均衡", "基础设施"),
				// 获取资源负载均衡器数据单条数据,
				beego.NSRouter("/:id:int", &lb.LbController{}, "get:LbData"),
				// 返回负载均衡服务所有数据,
				beego.NSRouter("/service/:hi(.*)", &lb.ServiceController{}, "get:ServiceData", "获取服务数据", "负载均衡详情", "负载均衡"),
			),
			beego.NSNamespace("/network",
				beego.NSNamespace("/lb",
					beego.NSNamespace("/service",
						// 负载均衡服务管理,
						beego.NSRouter("/:id:int", &lb.ServiceController{}, "delete:ServiceDelete", "删除服务数据", "负载均衡详情", "负载均衡"),
						// 负载均衡服务保存,
						beego.NSRouter("", &lb.ServiceController{}, "post:ServiceSave", "保存服务数据", "负载均衡详情", "负载均衡"),
					),
					beego.NSNamespace("/nginx",
						// 获取nginx配置
						beego.NSRouter("/:id:int", &lb.ServiceController{}, "get:GetNginxConf", "获取nginx配置数据", "负载均衡详情", "负载均衡"),
						// 保存nginx配置
						beego.NSRouter("/:id:int", &lb.ServiceController{}, "post:SaveNginxConf", "保存nginx配置数据", "负载均衡详情", "负载均衡"),
					),
					beego.NSRouter("/domain", &lb.ServiceController{}, "get:GetLbDomain"),
				),
				beego.NSNamespace("/cert",
					// 负载均衡服务管理,
					beego.NSRouter("/:id:int", &lb.CertController{}, "delete:CertDelete", "删除证书", "证书管理", "网络管理"),
					// 负载均衡服务保存,
					beego.NSRouter("", &lb.CertController{}, "post:CertSave", "保存证书", "证书管理", "网络管理"),
					// 获取资源负载均衡器数据所有数据,
					beego.NSRouter("", &lb.CertController{}, "get:CertData", "获取证书数据", "证书管理", "网络管理"),
				),
			),
		)
	...
	monitorApi :=
		beego.NewNamespace("/api/",
			beego.NSNamespace("/monitor",
				beego.NSNamespace("/scale",
					// 自动伸缩保存,
					beego.NSRouter("", &monitor.AutoScaleController{}, "post:AutoScaleSave", "自动伸缩保存", "伸缩配置", "自动伸缩"),
					// 删除自动伸缩
					beego.NSRouter("/:id:int", &monitor.AutoScaleController{}, "delete:AutoScaleDelete", "删除自动伸缩", "伸缩配置", "自动伸缩"),
					// 获取自动伸缩数据单条数据,
					beego.NSRouter("/:id:int", &monitor.AutoScaleController{}, "get:AutoScaleData", "获取自动伸缩数据单条", "伸缩配置", "自动伸缩"),
					// 获取自动伸缩数据,
					beego.NSRouter("", &monitor.AutoScaleController{}, "get:AutoScaleDatas", "获取自动伸缩数据", "伸缩配置", "自动伸缩"),
					// 获取自动伸缩数据,
					beego.NSRouter("/logs", &monitor.AutoScaleController{}, "get:AutoScaleLogsData", "获取自动伸缩日志", "伸缩配置", "自动伸缩"),
				),
			),
		)
